...
* [Association vs DB foreign-key audit](Associations.md) - Compare declared associations against the real database foreign keys
...
Audits whether your declared table associations (`belongsTo`, `hasMany`, `hasOne`, `belongsToMany`) agree with the actual database foreign keys, in both directions, across four layers — constraint, key-type, cascade-rule and loose-column — plus composite-key support. It is read-only and only suggests copy-paste fixes.

![Association audit matrix](img/associations_matrix.png)

A flat scan lists every finding at once, ordered worst-first with topic chips to mute whole categories:

![Association audit flat scan](img/associations_scan.png)

See [Association Audit Documentation](Associations.md) for the full guide: every check and its suggested fix, composite keys, the matrix and flat scan, and configuration.
